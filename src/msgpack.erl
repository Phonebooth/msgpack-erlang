...
-type options() ::
        [{spec, new|old} |
         ...
         {pack_str, from_binary|from_list|from_tagged_list|from_list_opt_tagged_list|none} |
         {allow_tagged_array_pack, boolean()} |
...
parse_options([{pack_str, From}|T], Opt)
  when From =:= from_binary orelse From =:= from_list orelse From =:= from_tagged_list orelse From =:= from_list_opt_tagged_list orelse From =:= none ->
    parse_options(T, Opt?OPTION{pack_str=From});

parse_options([{allow_tagged_array_pack, Bool}|T], Opt) when is_boolean(Bool) ->
    parse_options(T, Opt?OPTION{allow_tagged_array_pack=Bool});
